...
def decision_tree( data: pd.DataFrame, attr_val: str="" ) -> Node:

    classes = data[ GOAL ].unique()
    if len( classes ) == 1:
        return Node( is_leaf=True, val=f"{classes[ 0 ]} ({attr_val})" )

    attr = best_attr( data )
    new = Node( val=f"{attr}? ({attr_val}) ")
    for partition in split_by( data, attr ):
        new.children.append( decision_tree( partition ) )

    return new

decision_tree( CHART ).draw()
